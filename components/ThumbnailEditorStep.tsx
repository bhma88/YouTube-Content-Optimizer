...
export const ThumbnailEditorStep: React.FC<ThumbnailEditorStepProps> = ({ selectedTitle, topic, onBack }) => {
  ...
  return (
    <div>
      ...
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-bold mb-2">Step 4: Create & Customize Your Thumbnail</h2>
        <p className="text-lg text-gray-400 max-w-3xl mx-auto">Upload your photo, add style references, and use natural language to perfect your design.</p>
        <p className="font-semibold text-white mt-2">Selected Title: "{selectedTitle}"</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Controls Panel */}
        <div className="bg-brand-gray-light p-6 rounded-lg flex flex-col gap-6">
          <div>
          ...
          <div>
            <h3 className="text-xl font-semibold mb-3">2. (Optional) Add Style References</h3>
            <ImageUploader onFilesUploaded={setStyleImages} multiple={true} buttonText="Upload Thumbnails" />
          ...
          {analyzedStyle && (
            <div className="text-sm bg-brand-gray-dark p-3 rounded-md">
          ...
          <div>
             <button onClick={handleGenerate} disabled={isLoading || !faceImage} className="w-full bg-brand-red text-white font-bold py-3 px-6 rounded-md hover:bg-red-700 transition-colors disabled:bg-gray-500 text-lg">
                Generate Thumbnail
            </button>
          </div>
          {generatedThumbnail && (
            <div>
              ...
              <form onSubmit={handleEdit} className="flex gap-2">
                 <input type="text" value={editCommand} onChange={e => setEditCommand(e.target.value)} placeholder="e.g., 'Make the text bigger'" className="flex-grow bg-brand-gray-dark border border-brand-gray-medium rounded-md p-2 text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-brand-red" />
        ...
        <div className="flex flex-col gap-6">
          <div className="relative aspect-video bg-brand-gray-light rounded-lg flex items-center justify-center border-2 border-dashed border-brand-gray-medium overflow-hidden">
            {isLoading && (
          ...
          {generatedThumbnail && (
             <div className="flex justify-between items-center bg-brand-gray-light p-3 rounded-lg">
              <div className="flex gap-2">
                <button onClick={handleUndo} disabled={historyIndex <= 0} className="flex items-center gap-1.5 p-2 rounded-md bg-brand-gray-dark hover:bg-brand-gray-medium disabled:opacity-50 disabled:cursor-not-allowed"><UndoIcon /> Undo</button>
                <button onClick={handleRedo} disabled={historyIndex >= editHistory.length - 1} className="flex items-center gap-1.5 p-2 rounded-md bg-brand-gray-dark hover:bg-brand-gray-medium disabled:opacity-50 disabled:cursor-not-allowed"><RedoIcon /> Redo</button>
              </div>
               <button onClick={handleDownload} className="flex items-center gap-2 p-2 rounded-md bg-brand-red hover:bg-red-700 font-semibold"><DownloadIcon /> Download</button>
            </div>
          )}
